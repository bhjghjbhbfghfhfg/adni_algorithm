...
def val_model(epoch, val_data, ResNet_3D, log_best, state):
    ...
    with torch.no_grad():
        for (data1, data2, labels) in tqdm(val_data, total=n_batches):
            ...
            if args.state == 0:
                ...
                mri_pre.append(cls_mri)
                pet_pre.append(cls_pet)
                multi_pre.append(output)
                final_pre.append(output)
    ...
    plt.subplot(1, 1, 1)
    plt.plot(people.epoch, people.mri_pre, label="mri_pre")
    plt.plot(people.epoch, people.pet_pre, label="pet_pre")
    plt.legend()
    plt.xlabel("epoch")
    plt.ylabel("pre")

    plt.subplot(1, 2, 1)
    plt.plot(people.epoch, people.multi_pre, label="multi_pre")
    plt.plot(people.epoch, people.final_pre, label="final_pre")
    plt.legend()
    plt.xlabel("epoch")
    plt.ylabel("pre")
    if people.epoch == -1:
...
if __name__ == '__main__':
    ...
    if args.state != 0:
        ...
        ResNet_3D.load_state_dict(torch.load('./model/3DResNet.pt', map_location=torch.device('cpu')))
    else:
